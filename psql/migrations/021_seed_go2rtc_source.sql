...
UPDATE cameras SET go2rtc_source = 'rtsp://${go2rtc_username}:${go2rtc_password}@<LAN_IP>/h264Preview_01_main'
...
-- Amcrest
UPDATE cameras SET go2rtc_source = 'rtsp://${go2rtc_username}:${go2rtc_password}@<LAN_IP>/cam/realmonitor?channel=1&subtype=0'
  WHERE serial = 'AMC043145A67EFBF79' AND go2rtc_source IS NULL;

-- SV3C
UPDATE cameras SET go2rtc_source = 'rtsp://${go2rtc_username}:${go2rtc_password}@<LAN_IP>:554/stream1'
  WHERE serial = 'C6F0SgZ0N0PoL2' AND go2rtc_source IS NULL;

-- Eufy cameras (direct local RTSP, NOT eufy:// cloud P2P)
UPDATE cameras SET go2rtc_source = 'rtsp://${go2rtc_username}:${go2rtc_password}@<LAN_IP>/live0'
