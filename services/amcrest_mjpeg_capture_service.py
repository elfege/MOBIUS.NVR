...
class AmcrestMJPEGCaptureService:
    ...
    def restart_capture(self, camera_id: str, camera_config: dict) -> bool:
        ...
        """
        logger.info(f"[RESTART] Restarting Amcrest MJPEG capture for {camera_id}")
        ...
        try:
            ...
            if success:
                logger.info(f"[RESTART] Amcrest MJPEG restart successful for {camera_id}")
                return True
            else:
                logger.error(f"[RESTART] Amcrest MJPEG restart failed for {camera_id}")
                return False

        except Exception as e:
            logger.error(f"[RESTART] Amcrest MJPEG restart error for {camera_id}: {e}", exc_info=True)
            return False
