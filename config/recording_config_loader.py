...
class RecordingConfig:
        ...
        self.base_config = {
            "global_defaults": {
                "motion_recording": {
                    ...
                    "pre_buffer_enabled": False,  # Enable rolling segment pre-buffer
                    "pre_buffer_sec": 5,
                    "post_buffer_sec": 10,
                    "max_age_days": 7,
                    "quality": "main"
    ...
    def get_storage_paths(self) -> Dict[str, str]:
        ...
        Returns:
            Dict with keys:
            - recent_base: Container path for recent recordings (/recordings)
            - recent_host_path: Host path for UI display
            - archive_base: Container path for archive (/recordings/STORAGE)
            - archive_host_path: Host path for UI display
        """
        defaults = {
            'recent_base': '/recordings',
            'recent_host_path': '/mnt/sdc/NVR_Recent',
        ...
        stored = self.base_config.get('storage_paths', {})
    ...
    def get_migration_config(self) -> Dict[str, Any]:
        ...
        Returns:
            ...
            - age_threshold_days: Days before migrating from recent to archive
            - archive_retention_days: Days before deleting from archive
            - min_free_space_percent: Minimum free space before capacity-triggered migration
            - schedule_cron: Cron expression for scheduled migration
            - run_on_startup: Whether to run migration on app startup
        """
        defaults = {
            'enabled': True,
            'age_threshold_days': 3,
        ...
        stored = self.base_config.get('migration', {})
        return {**defaults, **stored}

    def is_migration_enabled(self) -> bool:
        """Check if storage migration is enabled."""
        return self.get_migration_config().get('enabled', True)

    def get_migration_age_threshold(self) -> int:
        """Get days before files are migrated from recent to archive."""
        return self.get_migration_config().get('age_threshold_days', 3)

    def get_archive_retention_days(self) -> int:
        """Get days before files are deleted from archive."""
        return self.get_migration_config().get('archive_retention_days', 90)
    ...
    def is_pre_buffer_enabled(self, camera_id: str) -> bool:
        """
        Check if pre-buffer recording is enabled for camera.

        Pre-buffer uses rolling segment capture to record footage from
        ...
        Returns:
            True if pre-buffer is enabled and pre_buffer_sec > 0
        """
        camera_cfg = self.get_camera_config(camera_id)
        motion_cfg = camera_cfg.get('motion_recording', {})
        pre_buffer_enabled = motion_cfg.get('pre_buffer_enabled', False)
        pre_buffer_sec = motion_cfg.get('pre_buffer_sec', 0)
        return pre_buffer_enabled and pre_buffer_sec > 0


    def get_pre_buffer_seconds(self, camera_id: str) -> int:
        """
        Get pre-buffer duration for camera.

        Args:
            camera_id: Camera identifier

        Returns:
            Pre-buffer duration in seconds (0 if disabled)
        """
        if not self.is_pre_buffer_enabled(camera_id):
            return 0
        camera_cfg = self.get_camera_config(camera_id)
        return camera_cfg.get('motion_recording', {}).get('pre_buffer_sec', 5)
