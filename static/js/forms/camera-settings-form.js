...
export class RecordingSettingsForm {
    ...
    static helpFor(key) {
        const HELP = {
            ...
            ui_health_monitor: { title: 'UI Health Monitor', description: 'When true, the frontend periodically pings this camera\'s stream and shows ONLINE/OFFLINE badges, and reacts to backend recovery events by refreshing the player. Disable for cameras that are intentionally offline, slow to respond, or where the recovery refresh produces false-positive "Signal Lost" overlays on a working stream.', dependencies: 'When false: no ONLINE/OFFLINE badges, no Signal Lost overlay from health-monitor triggers, and backend "recovered" events are ignored. ui_health_refresh_delay_ms has no effect.' },
            ui_health_refresh_delay_ms: { title: 'Health Refresh Delay (ms)', description: 'Milliseconds to wait after a backend recovery notification before triggering a stream refresh. 0 = immediate (current behavior). Higher values mask transient backend hiccups for streams that are actually fine. Suggested: 0 (default), 2000 (light debounce), 5000 (moderate), 10000 (aggressive). Range: 0–60000.', dependencies: 'Only applies when ui_health_monitor is true. Has no effect on stream-side errors (HLS error events, WebRTC PeerConnection failures) — only on backend-pushed recovery events.' },
