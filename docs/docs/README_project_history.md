...
- **Success Metrics**:
  - Login successful to camera <LAN_IP>
  - Snapshot working (319,693 bytes)
  - MJPEG stream active for Blue Iris client (<LAN_IP>)
...
- **Blue Iris Compatibility**: UniFi camera accessible at `http://<LAN_IP>:5000/api/unifi/g5flex_living/stream/mjpeg`
...
- **Working Configuration**:
  ...
  - URL: `http://<LAN_IP>:5000/api/streams/T8416P0023390DE9/playlist.m3u8`
...
- **Local Network Architecture**: All streaming operations happen locally on <LAN_IP>/24 network without cloud dependencies
...
- **Visual Architecture**: Graphviz diagrams provide immediate understanding of:
  - Network topology with IP addressing (<LAN_IP>/24)
...
- ✅ **Authentication working** to camera <LAN_IP>
- ✅ **MJPEG stream active** at `http://<LAN_IP>:8080/g5flex.mjpeg`
...
User needed to integrate newly installed UCKG2 Plus (<LAN_IP>) with existing containerized UniFi G5-Flex proxy to access LL-HLS streams instead of current MJPEG approach. Goal was adding UniFi Protect API as alternative streaming method alongside existing working MJPEG proxy.
...
**Implementation Plan**: Create local admin account on UCKG2 Plus (<LAN_IP>) with disabled remote access, then modify existing authentication scripts to use local credentials instead of cloud account. This approach eliminates the entire 2FA implementation challenge while maintaining security appropriate for local network access.
...
```
rtsps://<LAN_IP>:7441/{rtspAlias}?enableSrtp
```
...
class UniFiProtectService(CameraService):
    ...
    def get_stream_url(self) -> str:
        ...
        protect_ip = self.config.get('protect_ip', '<LAN_IP>')
...
- **Critical Change**: G5-Flex camera (68d49398005cf203e400043f) adopted into UniFi Protect, eliminating direct camera access at <LAN_IP>
- **New Access Point**: All camera operations must now go through Protect console at <LAN_IP>
...
- Assumed RTSPS (encrypted) required: `rtsps://<LAN_IP>:7441/{rtsp_alias}?enableSrtp`
...
- **Working URL**: `rtsp://<LAN_IP>:7447/zQvCrKqH0Yj5aslR`
- **Port**: 7447 (not 7441)
...
camera_info['protect_host']  # "<LAN_IP>"
```
...
3. Test FFmpeg transcoding: `rtsp://<LAN_IP>:7447/{rtsp_alias}` → HLS output
...
- **Working Format**: `rtsp://<LAN_IP>:7447/{rtsp_alias}` (no credentials, no query parameters)
...
if stream_type == "ll_hls" and camera_type == "unifi":
    ...
    protect_host = camera_info.get('protect_host', '<LAN_IP>')
...
- Format: `https://<LAN_IP>/proxy/protect/hls/{camera_id}/playlist.m3u8?token={auth_token}`
...
- **Working UniFi URL Format**: `rtsp://<LAN_IP>:7447/zmUKsRyrMpDGSThn` (no authentication, simple alias)
...
rtsp_url = handler.build_rtsp_url(camera, stream_type=stream_type)
assert rtsp_url == "rtsp://user:pass@<LAN_IP>:554/live0"
```
...
{
  "68d49398005cf203e400043f": {
    "protect_host": "<LAN_IP>",  // Repeated 10x
    "credentials": {
...
{
  "console": {
    "host": "<LAN_IP>"  // Once, shared by all cameras
  }
}
...
rtsp_url = camera_info['rtsp']['url']
# "rtsp://user:pass@<LAN_IP>:554/live0"
```
...
- UniFi source probe (example):
  `ffmpeg -rtsp_transport tcp -timeout 30000000 -i 'rtsp://<LAN_IP>:7447/<alias>' -frames:v 1 -y /tmp/kitchen_probe.jpg`
...
|--------|-----|-----|-----|--------|
| MEBO_CAMERA | <LAN_IP> | 68:39:43:BD:A5:6F | Yes | ✅ Streaming |
| CAT_FEEDER_CAM_2 | <LAN_IP> | E0:E2:E6:0C:50:F0 | Yes | ✅ Streaming |
| CAT_FEEDERS_CAM_1 | <LAN_IP> | 44:EF:BF:27:0D:30 | Yes | ✅ Streaming |
| Living_Reolink | <LAN_IP> | EC:71:DB:AD:0D:70 | Yes | ✅ Streaming |
| REOLINK_formerly_CAM_STAIRS | <LAN_IP> | b0:41:1d:5c:e8:7a | No | ✅ Streaming |
| CAM_OFFICE | <LAN_IP> | ec:71:db:3e:93:f5 | No | ✅ Streaming |
| CAM_TERRACE | <LAN_IP> | ec:71:db:c3:1a:14 | No | ✅ Streaming |
...
- TERRACE camera (RLC-410-5MP @ <LAN_IP>) failed to stream via FFmpeg
- OFFICE camera (identical model @ <LAN_IP>) streamed successfully
...
ffprobe -rtsp_transport tcp -i "rtsp://admin:password@<LAN_IP>:554/h264Preview_01_sub"
...
- Enabled RTMP on Reolink firmware (<LAN_IP>) and verified via `ffplay rtmp://…` (<1 s latency).
- Added backend route:
...
2. **WebRTC Prototype**
   ...
   - Configure RTSP → WebRTC relay for “CAMERA OFFICE” (<LAN_IP>) first.
...
- `REOLINK_TERRACE` (<LAN_IP>): Genuine hardware failure - `Connection refused`
...
- ❌ REOLINK_TERRACE (hardware failure - <LAN_IP> connection refused)
...
- **LL-HLS sidecar (MediaMTX)**
  ...
  - New config file: `packager/mediamtx.yml`
    ...
    - Path `REOLINK_OFFICE`:
      ...
      - `source: rtsp://admin:xxxxxxxxxxxxxxxxxxxxxxx@<LAN_IP>:554/h264Preview_01_sub`
...
ffmpeg -rtsp_transport udp -timeout 5000000 -analyzeduration 1000000 \
  ...
  -i rtsp://admin:PASSWORD@<LAN_IP>:554/h264Preview_01_sub \
...
```bash
nvr-edge      | <LAN_IP> - - [19/Oct/2025:06:25:12 +0000] "POST /api/stream/start/T8441P122428038A HTTP/2.0" 200 191 "https://<LAN_IP>/streams" "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/141.0.0.0 Safari/537.36" "-"
nvr-edge      | <LAN_IP> - - [19/Oct/2025:06:25:12 +0000] "POST /api/stream/start/REOLINK_OFFICE HTTP/2.0" 200 186 "https://<LAN_IP>/streams" "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/141.0.0.0 Safari/537.36" "-"
nvr-edge      | <LAN_IP> - - [19/Oct/2025:06:25:12 +0000] "POST /api/stream/start/REOLINK_TERRACE HTTP/2.0" 200 201 "https://<LAN_IP>/streams" "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/141.0.0.0 Safari/537.36" "-"
nvr-edge      | <LAN_IP> - - [19/Oct/2025:06:25:12 +0000] "POST /api/stream/start/REOLINK_LAUNDRY HTTP/2.0" 200 199 "https://<LAN_IP>/streams" "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/141.0.0.0 Safari/537.36" "-"
nvr-edge      | <LAN_IP> - - [19/Oct/2025:06:25:12 +0000] "GET /hls/REOLINK_OFFICE/index.m3u8 HTTP/2.0" 404 18 "https://<LAN_IP>/streams" "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/141.0.0.0 Safari/537.36" "-"
```
...
Diagnosed and resolved streaming issues with Reolink TERRACE camera (<LAN_IP>) through systematic hardware troubleshooting. Root cause identified as corroded RJ45 contacts from outdoor exposure. Discovered Reolink's proprietary Baichuan protocol (port 9000) and open-source Neolink bridge for ultra-low-latency streaming.
...
```bash
# Captured from <LAN_IP> (Windows PC) → <LAN_IP> (camera)
...
**Guinea Pig Selection:** Camera .88 (REOLINK_OFFICE @ <LAN_IP>)
...
- Cameras configured: 2
  - REOLINK_OFFICE (<LAN_IP>:9000)
  - REOLINK_TERRACE (<LAN_IP>:9000)
...
2. **Baichuan connection successful:**
   ...
   ```
   [INFO] REOLINK_OFFICE: TCP Discovery success at <LAN_IP>:9000
...
- **2 Reolink RLC-410-5MP cameras**:
  - REOLINK_OFFICE (<LAN_IP>)
  - REOLINK_TERRACE (<LAN_IP>)
...
- **REOLINK_OFFICE** (<LAN_IP>) - RLC-410-5MP - Primary test subject
...
- REOLINK_OFFICE (<LAN_IP>) - RLC-410-5MP
- REOLINK_TERRACE (<LAN_IP>) - RLC-410-5MP
...
curl --digest -u "admin:password" "http://<LAN_IP>/cgi-bin/ptz.cgi?action=start&channel=0&code=Right&arg1=0&arg2=5&arg3=0"
curl --digest -u "admin:password" "http://<LAN_IP>/cgi-bin/ptz.cgi?action=stop&channel=0&code=Right&arg1=0&arg2=0&arg3=0"
```
...
2. `cameras.json`
   ...
   - Host: <LAN_IP>, MAC: EC:71:DB:AD:0D:70
...
✅ URL built: rtsp://admin:01234567@<LAN_IP>:554/12  # WRONG PASSWORD
```
...
```bash
   $ ffplay -rtsp_transport tcp -timeout 5000000 \
     rtsp://admin:TarTo56%29%29%23FatouiiDRtu@<LAN_IP>:554/12
...
- Camera IP: <LAN_IP> (per cameras.json)
