...
## Session: January 3-4, 2026 (19:32-02:30+ EST) - UNDOCUMENTED BY PREVIOUS THREAD

### Branch: `stream_watchdog_backend_restart_JAN_3_2026_a`

**Context Note**: Previous Claude thread failed to update handoff documentation during this entire session despite multiple file modifications and debugging. This section reconstructed from chat history.

---

### Stream Watchdog Investigation - FFmpeg Reconnect Flags

#### Objective

Investigate stream watchdog fixes: "proper backend restart when a stream fails while camera still network reachable"

#### Investigation Approach (Step-by-Step)

1. **Test FFmpeg 7.1.3 reconnect flags** before implementation
2. Verify flags don't cause crashes (unlike old FFmpeg 5.1.6)
3. If successful, add to cameras.json `rtsp_input` section

#### FFmpeg Test Command (Final Correct Version)
...
ffmpeg -rtsp_transport tcp \
  ...
  -reconnect_on_network_error 1 \
  -reconnect_delay_max 5 \
  -i rtsp://${EUFY_CAMERA_T8416P0023370398_USERNAME}:${EUFY_CAMERA_T8416P0023370398_PASSWORD}@192.168.10.87:554/live0 \
  -t 30 \
  -f null -
```
...
#### Errors Made During Session (Documentation of Issues)

1. **Invented credential variable names** - Used `SV3C_LIVING_3_USERNAME` which doesn't exist. Correct pattern: `SV3C_USERNAME`/`SV3C_PASSWORD` (shared) and `EUFY_CAMERA_<SERIAL>_USERNAME`/`PASSWORD` (per-camera)

2. **Wrong IP address (Bridge Obsession)** - Used 192.168.10.101:8554 (eufy-security-ws bridge) instead of camera's direct IP 192.168.10.87:554. General documentation mentioned "Eufy cameras require eufy-security-ws bridge" but T8416P0023370398 connects directly per cameras.json config.

3. **Failed to update handoff documentation** - Multiple context compactions occurred and handoff was never updated throughout the session.

#### Key Lesson

Always read specific camera configuration in cameras.json first, don't assume based on general documentation. Use `jq` to extract exact values:

```bash
jq '.devices.<SERIAL>.rtsp' config/cameras.json
```
