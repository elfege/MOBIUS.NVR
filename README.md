...
- **Multi-Vendor Support**: Eufy, Reolink, UniFi Protect, Amcrest, SV3C — plus a **generic ONVIF** type for any ONVIF camera with no dedicated handler. The RTSP path is auto-discovered via ONVIF GetProfiles + GetStreamUri (not hardcoded per vendor), and the stream is `ffprobe`d so `ll_hls` is built **codec-aware**: H.264 main streams pass through (`copy`), HEVC/H.265 are transcoded to H.264 (Chrome can't play HEVC over WebRTC/HLS). A **Re-discover** button (Settings → Cameras) re-runs that probe for an existing camera
...
- **Native sub-stream (per-camera CPU optimization)**: By default the low-res grid tile is produced by decoding the camera's (often 4K) main stream and re-encoding a small sub via `libx264` — the single biggest CPU cost on the box. A background **multi-stream capability test** measures whether a camera can serve a *second* concurrent stream (its own native sub profile) without degrading the main; when it can (`multistream_ok`), the operator may set that camera's **Grid Sub Source** to `native`, and the tile is served by **copying** the camera's native sub (zero decode/encode). The choice is **per-camera and operator-made** — `native` is only offered on a camera that passed the test, is never auto-selected, and any camera left on the default `transcode` behaves exactly as before. The settings panel shows the verdict, measured fps, and a suggestion when native would help. For cameras whose sub stream ONVIF discovery can't see (Eufy, SV3C, Reolink E1, other budget brands), a per-camera **Sub Stream Path** field lets the operator type the sub endpoint (e.g. `/live1`); the capability test then taps that path to prove whether the camera actually serves it.
...
- **Health Monitoring**: Backend watchdog with exponential backoff + frontend freeze detection based on **decoded-frame progress** (`getVideoPlaybackQuality` for video, pixel signature for MJPEG) — catches the "frozen but connected" case where the video clock keeps ticking while no new frame decodes, which a `currentTime`-based check misses. The latency badge reads the same signal and shows **STALLED** instead of a misleading low latency when frames stop arriving.
- **Per-User Preferences**: Stream type, display order, camera visibility, grid layout mode, video fit — all per-user in DB
- **Grid Layout Modes**: Uniform, last-row stretch, auto-fit, masonry — with 3 video fit options (cover/contain/fill). Video fit is driven by a CSS custom property (`--video-fit`) on the grid container so the account default applies declaratively to every tile — including stream elements recreated by a reconnect — and survives refresh; changing the **global** video fit **overrides and clears any per-camera fit overrides** so it applies uniformly
- **Camera Management UI** (admin): **Settings → Cameras** lists every configured camera (name, serial, type, stream type, hub, host) from the DB-authoritative source; **adds** new RTSP/ONVIF cameras (amcrest/sv3c/reolink/unifi/**onvif** — inserts the row, seeds the full standard field set so a UI-added camera is configured exactly like a `cameras.json`-seeded one, stores encrypted credentials, and goes live immediately; for the generic **onvif** type it also runs ONVIF GetProfiles+GetStreamUri discovery at add-time to learn the camera's RTSP path; Eufy uses the Eufy Bridge); **removes** a camera via a **soft-delete** (marks `deleted_at`, stops the stream, hides it from the grid/list — no hard delete, no FK cascade; recoverable by re-adding the same serial, which reactivates the row, and it stays removed across `cameras.json` re-seeds); and runs a basic **LAN scan** that probes the camera subnet for RTSP/ONVIF devices not yet added, with an **Add** button on each discovered device (discovery derives the subnet from known camera IPs). Each listed camera also has a **settings gear** that opens the same per-camera settings modal the grid tiles use — the only UI path to a server-**hidden** camera (which the navbar "Filter cameras" selector, built from rendered tiles, cannot reach), e.g. to flip it back into the grid. The stream-type column is shown with normalized labels (LL-HLS / WebRTC / go2rtc / MJPEG)
- **UI Design Tokens in PostgreSQL** (`ui` schema): every tunable UI value — badge colors, the signal-lost black-out, action-bar idle opacity, freshness thresholds — lives in `ui.tbl_css_tokens` (audited: who changed which color when) and is applied as a `:root` style block by a zero-FOUC injector that hot-refreshes every 60s, so `UPDATE ui.tbl_css_tokens SET value='#ff00ff' WHERE token='--badge-live-color'` restyles every open kiosk without a reload; stylesheets keep `var(--x, fallback)` defaults so a DB outage costs nothing. A generated **CSS class index** (`ui.tbl_css_classes`, rebuilt by a crawler at every `start.sh`) makes the whole class inventory queryable — including dead CSS and 'ghost' classes referenced in JS/templates but defined nowhere
- **Standardized Advanced Settings** (per-camera): the settings modal's **Advanced** tab renders **preset controls** — dropdowns for enum fields (stream type, streaming hub, power supply, tracking owner, camera type), toggles for booleans, bounded-number inputs (min/max), read-only chips for **capabilities** (which are hardware-derived — a **Re-discover** action reconciles them against ONVIF rather than hand-editing), and a JSON editor only where a field is genuinely structured. Every control, its domain and its tooltip come from **one source of truth** (`services/camera_fields.py`) served via `GET /api/schema/camera-fields`. A **compatibility matrix** greys out invalid option combinations (e.g. WebRTC requires the go2rtc hub; Eufy is incompatible with go2rtc) and the same rules **reject invalid combinations server-side** with a clear reason Structured JSONB blobs are hardened at **three layers**: typed sub-forms with FFmpeg-aware cross-parameter greying + plain-language tooltips (every encoder knob greys out when `c:v=copy`, reason on hover), server-side inner-shape validation with precise error paths (`mjpeg_snap.main.fps: expected a number`), and PostgreSQL CHECK constraints on the JSONB columns themselves. Machine-generated configs (`ll_hls`, `rtsp_input/output`) carry **⟲ Reset to defaults** (cameras.json seed / vendor generators) instead of inviting hand-edits
...
- **`services/camera_fields.py`** is the single **field registry** — one `FieldSpec` per camera field declares its storage class, default, UI control, enum domain, numeric bounds, tooltip and editability. The seed-sync field lists (`camera_config_sync.py`), the DB→cache reader (`camera_repository.py`), the settings write-router (`settings.py`) and the settings-PUT whitelist (`routes/camera.py`) all **derive** from it, plus the UI schema endpoint and the compatibility matrix. This replaced four hand-maintained allow-lists that used to silently desync
...
| `/api/cameras` | POST | Admin: add a camera (RTSP/ONVIF vendors — amcrest/sv3c/reolink/unifi/onvif). Inserts the row, stores encrypted credentials, seeds the streaming-critical config; for `onvif` it runs ONVIF GetStreamUri + ffprobe discovery to learn the RTSP path + codec. Reloads the live repo. Eufy is P2P (use the Eufy Bridge) |
| `/api/cameras/<serial>/rediscover` | POST | Admin: re-run ONVIF discovery (GetStreamUri + ffprobe) for an existing generic `onvif` camera and rebuild the codec-aware `ll_hls` (passthrough H.264 / transcode HEVC). Returns the resolved tiers, main codec + resolution, and whether it transcodes |
| `/api/cameras/<serial>` | DELETE | Admin: **soft-delete** a camera (sets `deleted_at`, stops its stream; no hard delete / no FK cascade — history + credentials retained). Recoverable: re-adding the same serial via POST reactivates the row. Survives `cameras.json` re-seeds (the sync never resets `deleted_at`) |
...
| `/api/camera/<serial>/settings` | PUT | Update per-camera settings. Enforces the editable-field whitelist, runs server-side enum + compatibility-matrix validation (rejects invalid combos with a reason), routes a `capabilities` edit through ONVIF re-discovery, and refuses `substream_source=native` on a camera that has not passed the multi-stream capability test |
| `/api/schema/camera-fields` | GET | The camera-field UI schema (control type, enum domain, numeric bounds, tooltip per field) + the compatibility matrix. Drives the settings form's preset controls. Structure from `services/camera_fields.py`; the human TEXT is overlaid from the DB (see below) |
| `/api/schema/field-help` | GET / PUT | The **data-driven** field tooltips/descriptions (`ui.tbl_field_help`). GET lists every entry; PUT (admin) edits one live — title/description/dependencies/sub-key help — with no code deploy. `ui_schema()` overlays these onto the registry structure, falling back to the registry default if a row is absent |
| `/api/cameras/<serial>/multistream-test` | POST/GET | Admin: run (POST) or read the stored verdict (GET) of the **multi-stream capability test** — measures whether a camera can serve a 2nd concurrent stream (its native sub) without degrading the main. GET also returns the `nudge` flag |
| `/api/multistream-test/<job_id>` | GET | Admin: short-poll an in-flight capability-test job (state + line-by-line log + verdict) |
...
| `/api/cameras/clear-video-fit-overrides` | POST | Clear all per-camera `video_fit_mode` overrides (invoked when the global video fit is changed, so it applies uniformly) |
