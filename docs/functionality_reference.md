...
| `STREAMS.DEAD_TILE.ESSENTIAL_CONTROLS` | Tile carries `.signal-lost` class (dead stream, grid layout) | settings + power + playback + **controls-toggle** icons stay visible (B2 essential set; controls-toggle added 2026-06-19 so operator can RESTART a dead stream); audio / PTZ / talkback / fullscreen / record / more stay hidden ("meaningless on a dead stream" — operator policy 2026-05-23). Effective opacity walked through ancestor chain. | [static/css/components/stream-item.css:446-499](../static/css/components/stream-item.css), `tests/regression/ledger.yaml#dead-tile-button-opacity-inheritance` | e2e:PASS (matrix, 10 cells; mutation-tested) |
| `STREAMS.DEAD_TILE.EXPANDED_ESSENTIAL_CONTROLS` | Tile carries `.signal-lost` AND `.expanded` (dead stream, expanded layout) | Same B2 essential set as grid + signal-lost (settings/power/playback/controls-toggle visible; rest hidden). Separate cell because expanded mode has its own `!important` layer in stream-control-bar.css; catches regressions that fix one cell while breaking the other. | [static/css/components/stream-item.css](../static/css/components/stream-item.css), [static/css/components/stream-control-bar.css](../static/css/components/stream-control-bar.css) | e2e:PASS (matrix, 10 cells) |
| `STREAMS.EXPAND.LIVE_BAR_VISIBLE` | Tile is `.expanded` (single-camera full-window view), stream live | All 10 action-bar icons visible by default — no hover-to-reveal. Production rule: expanded bar is persistent (operator doesn't need to hover-hunt in full-screen view). | [static/css/components/stream-control-bar.css](../static/css/components/stream-control-bar.css) | e2e:PASS (matrix, 10 cells) |
| `STREAMS.PIN.LIVE_BAR_VISIBLE` and `STREAMS.PIN.SIGNAL_LOST_ESSENTIAL` | Tile is `.pinned-window` (popped-out floating window), live or signal-lost | Live: all 10 icons persistently visible (no hover). Signal-lost: same B2 essential set as grid/expanded (settings/power/playback/controls-toggle visible; rest hidden). Sees same layout-agnostic signal-lost CSS at stream-item.css:458-497. | [static/css/components/fullscreen.css:228-253](../static/css/components/fullscreen.css), [static/css/components/stream-item.css:458-497](../static/css/components/stream-item.css) | e2e:PASS (matrix, 20 cells total) |
| `STREAMS.CSS_FULLSCREEN.LIVE_BAR_VISIBLE` and `STREAMS.CSS_FULLSCREEN.SIGNAL_LOST_ESSENTIAL` | Tile is `.css-fullscreen` (browser-true Fullscreen API), live or signal-lost | Live: all 10 icons persistently visible. Signal-lost: B2 essential set surfaces; rest hidden. The css-fullscreen layout adds its own `!important` layer at fullscreen.css:306-316 that the matrix catches separately from `.expanded` and `.pinned-window`. | [static/css/components/fullscreen.css:306-316](../static/css/components/fullscreen.css), [static/css/components/stream-item.css:458-497](../static/css/components/stream-item.css) | e2e:PASS (matrix, 20 cells total) |
| `STREAMS.RECORD.CLICK_NO_404` | Operator clicks the record icon on a live tile | Frontend builds `POST /api/recording/<serial>/start`; backend returns 2xx with `recording_id`. Regression-guarded: pre-2026-06-19 the frontend built `/start/<id>` while backend wanted `/<id>/start` — 404 in prod. Httpx tests didn't catch it (they hand-coded the correct URL); only a browser-driven click test does. | [static/js/controllers/recording-controller.js:66](../static/js/controllers/recording-controller.js), [routes/recording.py](../routes/recording.py) | e2e:PASS (skip when no tile rendered) |
| `STREAMS.POWER.CLICK_NO_404` | Operator clicks the power icon on a `.power-configured` tile | Frontend builds `POST /api/power/<serial>/cycle` (hubitat) or `/api/poe/<serial>/cycle` (POE). Backend returns 2xx with `{success: true}`. Same browser-driven click pattern as record — guards against future segment-order or method drifts. | [static/js/controllers/power-controller.js:88](../static/js/controllers/power-controller.js), [routes/power.py](../routes/power.py) | e2e:PASS (skip when no tile rendered) |
| `STREAMS.PTZ.CLICK_NO_404` | Operator opens PTZ panel, presses `data-direction="left"` button | Frontend builds `POST /api/ptz/<serial>/left`; backend returns 2xx with `{success: true}`. **Eufy SKIP rule** (2026-06-19): when ALL three hold — camera type=`eufy` AND `/api/eufy/cloud-status` returns `cloud_reachable: false` AND backend also reports failure — the test SKIPs (environmental, not a regression). Any partial match = real fail. | [static/js/controllers/ptz-controller.js:789](../static/js/controllers/ptz-controller.js), [routes/ptz.py](../routes/ptz.py), [routes/talkback.py:601](../routes/talkback.py), `memory/feedback_eufy_ptz_skip_when_cloud_down_and_backend_failed` | e2e:PASS (skip when no PTZ tile rendered) |
| `STREAMS.FREEZE.DETECT_STALE_NONBLACK` | iOS WebKit holds last frame on stall — tile is NOT black so the canvas blank-pixel sampler in `makeHealthMonitor` never trips; also iOS throttles the 6 s `setInterval` under thermal / background. Same symptom appears on Chrome when camera-side encoder sends repeated identical frames (Defect 3). | NEW (2026-06-20): `static/js/streaming/freeze-watchdog.js` — separate per-tile poller (2s cadence, 6s stall threshold, 10s warmup) that toggles `.signal-lost` purely based on `<video>.currentTime` progression. Independent of HealthMonitor (which keeps its slower restart-cascade timing for the recovery side). Detection lag: ~16s worst case (down from HealthMonitor's ~86s). Cross-browser stable; works on iOS Safari. | [static/js/streaming/freeze-watchdog.js](../static/js/streaming/freeze-watchdog.js), [static/js/streaming/stream.js:attachHealthMonitor](../static/js/streaming/stream.js), `memory/project_frozen_stream_no_buttons_ipad_health_monitor` | e2e:PASS (2 cells: freeze→signal-lost + paused→no-tag) |
| `STREAMS.SNAP_ONLY_BADGE.NATIVE_MJPEG` | Tile carries `data-streaming-hub="native_mjpeg"` (camera in snap-only mode — broken RTSP, falls back to vendor-MJPEG snapshot polling) | A small amber **SNAP** badge surfaces in the upper-left of the tile. Persistent (not hover-only), low-opacity (0.7), positioned to avoid the latency badge + status indicators. Pure CSS attribute-selector; zero JS. Stays visible on `.signal-lost` tiles too — being in snap-only mode is camera-config metadata, not a status signal. | [static/css/components/stream-item.css](../static/css/components/stream-item.css), [templates/streams.html:454](../templates/streams.html) | e2e:PASS (4 cells: native_mjpeg shows / mediamtx hides / go2rtc hides / signal-lost+native_mjpeg stays visible) |
