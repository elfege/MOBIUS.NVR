...
- XCPTP369388MNVTG (Living_REOLINK): Direct RTSP to <LAN_IP>
- 95270001CSO4BPDZ: Direct RTSP to <LAN_IP>
...
ffmpeg -rtsp_transport tcp \
  ...
  -reconnect_on_network_error 1 \
  -reconnect_delay_max 5 \
  -i rtsp://${EUFY_CAMERA_T8416P0023370398_USERNAME}:${EUFY_CAMERA_T8416P0023370398_PASSWORD}@<LAN_IP>:554/live0 \
  -t 30 \
  -f null -
```
...
- RTSP URL: `rtsp://<credentials>@<LAN_IP>:554/live0`
- Config source: `jq '.devices.T8416P0023370398.rtsp' config/cameras.json` returns `{"host": "<LAN_IP>", "port": 554, "path": "/live0"}`
...
2. **Wrong IP address (Bridge Obsession)** - Used <LAN_IP>:8554 (eufy-security-ws bridge) instead of camera's direct IP <LAN_IP>:554. General documentation mentioned "Eufy cameras require eufy-security-ws bridge" but T8416P0023370398 connects directly per cameras.json config.
