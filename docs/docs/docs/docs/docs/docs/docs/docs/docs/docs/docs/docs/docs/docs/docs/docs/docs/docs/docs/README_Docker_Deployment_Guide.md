...
- UniFi Protect Console (UCKG2 Plus) at <LAN_IP>
- Local admin account created on Protect (username: `user-api`)
- Network access to cameras on <LAN_IP>/24
...
```
app.py → unifi_service.py → http://<LAN_IP>/api/1.1/login
                           → http://<LAN_IP>/snap.jpeg
```

### After (Protect API)
```
app.py → unifi_protect_service.py → https://<LAN_IP>/api/auth/login
                                   → https://<LAN_IP>/proxy/protect/api/cameras/{id}/snapshot
```
...
# API status endpoint
curl http://<LAN_IP>:5000/api/status

# Individual camera stats
curl http://<LAN_IP>:5000/api/unifi/68d49398005cf203e400043f/stats
```
...
curl -k -b cookies.txt https://<LAN_IP>/proxy/protect/api/bootstrap | jq '.cameras[] | {id, name}'
```
...
```bash
curl -k -b cookies.txt https://<LAN_IP>/proxy/protect/api/cameras/68d49398005cf203e400043f/snapshot -o test.jpg
```
...
python3 -c "from services.unifi_protect_service import UniFiProtectService; \
    config={'name':'Test','protect_host':'<LAN_IP>','camera_id':'68d49398005cf203e400043f','credentials':{'username':'x','password':'x'}}; \
...
2. **Network/IP:** <LAN_IP> (or Docker host IP)
3. **Port:** 5000
