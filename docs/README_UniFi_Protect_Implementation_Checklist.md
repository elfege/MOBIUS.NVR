...
curl -k -b ~/0_UNIFI_NVR/cookies/cookies.txt \
  https://<LAN_IP>/proxy/protect/api/bootstrap | \
...
- [ ] Test health: `curl http://<LAN_IP>:5000/api/status`
- [ ] Test snapshot: `curl http://<LAN_IP>:5000/api/unifi/68d49398005cf203e400043f/snapshot -o test.jpg`
- [ ] Open test.jpg: Should show camera image

### Expected Log Output
```
✓ Loaded UniFi camera: G5 Flex
Logging into Protect at <LAN_IP> for camera G5 Flex
...
```bash
# Status
curl http://<LAN_IP>:5000/api/status | jq

# Camera list
curl http://<LAN_IP>:5000/api/unifi/cameras | jq

# Snapshot
curl http://<LAN_IP>:5000/api/unifi/68d49398005cf203e400043f/snapshot -o test.jpg

# MJPEG stream (should download continuously)
curl http://<LAN_IP>:5000/api/unifi/68d49398005cf203e400043f/stream/mjpeg --output - | head -c 10000000 > stream_test.mjpeg
```
...
- [ ] Add entry to `DOCS/README_project_history.md`:
  ...
  - Camera adopted into UCKG2 Plus (<LAN_IP>)
  ...
  - Auth endpoint: https://<LAN_IP>/api/auth/login
  - Snapshot: https://<LAN_IP>/proxy/protect/api/cameras/{id}/snapshot
...
curl -k -X POST https://<LAN_IP>/api/auth/login \
...
```bash
# List all cameras
curl -k -b cookies.txt https://<LAN_IP>/proxy/protect/api/bootstrap | \
